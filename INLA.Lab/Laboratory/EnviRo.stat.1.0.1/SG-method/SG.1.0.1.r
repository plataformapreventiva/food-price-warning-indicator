...
# The functions in this file are the same as those in Version 1.0
     ...
     dims = 2., lambda = 0., dev.mon = "postscript", ncoords)
{
	...
	eval(call(dev.mon, file = "ps.objf.monitor"))
	dev.mon <- dev.cur()
	dev.set(dev.plot)
        maxdiff = 1
	par(mfrow = c(1., 2.))
	...
	variogfit <- Fvariogfit3(disp.lt, h.lt, model = model, a0 = a0, t0 = t0,
		bep = BEpenalty, max.iter = max.iter, max.fcal = max.fcal)
	#
	# 
	...
	a0 <- variogfit$a[1.]
	t0 <- variogfit$t0
	temp <- setplot(rbind(coords, ncoords[, 1.:2.]), ax = T)
	text(coords,labels=c(1:(dim(coords)[1])))
	if(dims == 3.) {
		symbols(ncoords[, 1.], ncoords[, 2.], circles = (ncoords[,3.] - 
             min(ncoords[, 3.]) + 0.01), inches = 0.15, add = T)
	}
	arrows(coords[, 1.], coords[, 2.], ncoords[, 1.], ncoords[, 2.], length=0.075 )
	...
	plot(h.lt, disp.lt, xlab = "D-plane distance", ylab = paste(
		"Dispersion, rmsre =", format(round(rmsre, 4.))), 
        xlim = c(0., max(h.lt)), ylim = c(0., max(disp.lt)))
	title(main = paste(ifelse(model == 1., " Exponential", " Gaussian"),
		"Fitted Variogram"), cex = 0.75)
	title(main = paste("\ncriterion = ", format(round(objf, 4.))), cex = 
		0.75)
	...
	while(i <= alter.lim & maxdiff > tol) {
		...
		text(coords,labels=c(1:(dim(coords)[1])))
		if(dims == 3.) {
			symbols(ncoords[, 1.], ncoords[, 2.], circles = (
				ncoords[, 3.] - min(ncoords[, 3.]) + 0.01),
				inches = 0.15, add = T)
		           }
		arrows(coords[, 1.], coords[, 2.], ncoords[, 1.], ncoords[, 2.], length=0.075)
		...
		a0 <- variogfit$a[1.]
		t0 <- variogfit$t0
		rmsre <- sqrt(mean(((disp.lt - variogfit$fit)/variogfit$fit)^
			2.))
		plot(h.lt, disp.lt, xlab = "D-plane distance", ylab = paste(
			"Dispersion, rmsre =", format(round(rmsre, 4.))), xlim
			 = c(0., max(h.lt)), ylim = c(0., max(disp.lt)))
		title(main = paste(ifelse(model == 1., " Exponential", 
			" Gaussian"), "Fitted Variogram"), cex = 0.75)
		title(main = paste("\ncriterion = ", format(round(objf, 4.))),
			cex = 0.75)
		abline(h = 2., lty = 2.)
		#
		lines(h.lt[order(h.lt)], variogfit$fit[order(h.lt)])
...
Ftransdraw <- function(disp, Gcrds, MDScrds, gridstr, sta.names, lambda = 0., lsq = F, eye,
...
# - compute and draw image of regular grid (2D or 3D perspective plot)
	...
	grid.nm <- !#
	is.na(grid[, 1.])
	plim <- setplot(grid[, 1.], grid[, 2.], ax = T)
	...
	#	junk <- locator(n=1)
		if(Ddim == 2.) {
		plim <- setplot(Dgrid[, 1.], Dgrid[, 2.], ax = T)
		...
		title(main = "D-plane Coordinates", 
			...
			persp.setup(lty = 1.:3., col = c(1., 1., 1.), lwd = c(0., 0., 0.))
			while(length(eye) == 3.) {
				...
				lp.out <- persp(Dinterp$x, Dinterp$y, Dinterp$
					z, eye = eye)
				Dcrds.pp <- perspp(Dcrds[, 1.], Dcrds[, 2.],
					Dcrds[, 3.], lp.out)
				#
				text(Dcrds.pp$x, Dcrds.pp$y, sta.names, cex = 
					0.75)
				Dgrid.pp <- Dgrid[, 1.:2.]
				temp.pp <- perspp(Dgrid[grid.nm, 1.], 
                                              Dgrid[grid.nm, 2.], 
	...
	prt = 0., dims = 2., lambda = 0., bem)
{
	...
	#
	cat("MDS-convergence: ", temp$converged, "\n")
	#
	cat("  nlm code =  ", temp$conv.type, "\n")
...
Fvariogfit3 <- function(disp.lt, h.lt, model = 1., a0 = 0.1, t0 = .5,
                 max.iter = 25., max.fcal= 100., bep = 0.)
{
	...
	a0 <- exp(temp$x[2.])
	a1 <- 2. - a0
	cat("VAR-convergence: ", temp$converged, "\n")
	cat("  nlm code =  ", temp$conv.type, "\n")
...
Disp.link <- function(disp.mx, coords, dmap, ddisp, names, device ="X11")
 {
