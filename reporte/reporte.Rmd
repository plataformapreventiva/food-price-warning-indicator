...
$$y_n ∼ Normal(α + βy_n−1, σ ).$$

  This model introduces a coefficient, which we’ll call β. The parameter φ controlls the degree to
  which the random walk reverts to the mean – when β = 1, the model is identical to the random walk, but
  at smaller values, the model will revert back to the mean (which in this case is zero). Also, φ can take on
  negative values
